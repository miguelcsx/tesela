//! Top-level `App` builder for defining a complete Lattice ontology.

use lattice_compiler::{CompileResult, Compiler};
...
/// A fluent builder for a complete Lattice ontology definition.
///
...
impl App {
    ...
    pub fn into_runtime(
        self,
        opts: lattice_runtime::runtime::RuntimeOptions,
    ) -> Result<std::sync::Arc<lattice_runtime::runtime::Runtime>, Error> {
        let result = self.compile();
        let spec = result.spec.ok_or_else(|| {
        ...
        lattice_runtime::runtime::Runtime::new(spec, opts)
    }
}
