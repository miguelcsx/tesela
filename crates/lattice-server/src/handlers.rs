//! Axum route handler functions for all Lattice HTTP endpoints.
...
use lattice_ir::{AggregateResult, Branch, MutationResult, Page, PipelineResult, Record, Spec};
use lattice_runtime::{
...
pub(crate) struct SearchBody {
    #[serde(default)]
    pub filter: Option<lattice_ir::Filter>,
    #[serde(default)]
    pub sort: Vec<Sort>,
    #[serde(default)]
...
) -> Result<Json<lattice_ir::UploadResult>, ApiError> {
    let actor = extract_actor(&state, &headers)?;
    let flow = parse_api_name(&name)?;
...
pub(crate) async fn explain_handler(
    ...
    Path(type_name): Path<String>,
    Json(body): Json<SearchBody>,
) -> Result<Json<lattice_ir::ExplainPlan>, ApiError> {
    let actor = extract_actor(&state, &headers)?;
    let obj = parse_api_name(&type_name)?;
    let query = Query {
        filter: body.filter,
...
) -> Result<Json<lattice_ir::ActionResult>, ApiError> {
    let actor = extract_actor(&state, &headers)?;
...
) -> Result<Json<lattice_ir::AgentRun>, ApiError> {
    let actor = extract_actor(&state, &headers)?;
...
pub(crate) async fn explain_traverse_handler(
    ...
    Ok(Json(lattice_ir::ExplainPlan { steps }))
}
...
pub(crate) async fn object_set_compose_handler(
    ...
    let op = match body.op.as_deref() {
        Some("intersect") => lattice_ir::SetOp::Intersect,
        Some("subtract") => lattice_ir::SetOp::Subtract,
        _ => lattice_ir::SetOp::Union,
    };
    Ok(Json(
...
pub(crate) async fn pipeline_execute_handler(
    ...
    let mode = match body.mode.as_deref() {
        Some("snapshot") => lattice_ir::ExecutionMode::Snapshot,
        _ => lattice_ir::ExecutionMode::Incremental,
    };
