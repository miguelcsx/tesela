...
use lattice_core::{ApiName, Error, PolicyEffect};
use lattice_ir::PolicyRule;
    ...
    fn resolve_roles(&self, actor: &Actor) -> HashSet<String> {
        ...
        while let Some(role_str) = queue.pop() {
            if resolved.insert(role_str.clone()) {
                if let Ok(role_name) = role_str.parse::<ApiName>() {
                    if let Some(parents) = self.role_inheritance.get(&role_name) {
...
fn merge_and_filter(
    left: Option<lattice_ir::Filter>,
    right: lattice_ir::Filter,
) -> lattice_ir::Filter {
    match left {
        Some(l) => lattice_ir::Filter {
            op: lattice_core::FilterOp::And,
            field: None,
            value: None,
            values: Vec::new(),
            args: vec![l, right],
...
impl ConditionEvaluator for AlwaysTrueConditionEvaluator {
    fn evaluate(
        &self,
        _condition: &str,
        _context: &BTreeMap<String, lattice_core::Value>,
    ) -> Result<bool, Error> {
        Ok(true)
    }
}
...
impl ObligationExecutor for NoopObligationExecutor {
    fn execute(
        &self,
        _obligation: &lattice_ir::Obligation,
        _context: &BTreeMap<String, lattice_core::Value>,
    ) -> Result<(), Error> {
        Ok(())
    }
