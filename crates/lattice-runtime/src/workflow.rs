...
pub type Guard =
    fn(data: &std::collections::BTreeMap<String, lattice_core::Value>) -> Result<(), Error>;
...
/// # Example
/// ```rust
/// use lattice_runtime::workflow::{Machine, State, TransitionDef};
///
...
impl Machine {
    ...
    pub fn transition(
        ...
        from: &State,
        event: &str,
        data: &std::collections::BTreeMap<String, lattice_core::Value>,
    ) -> Result<State, Error> {
        let t = self
            .index
            .get(&TransitionKey {
...
mod tests {
    ...
    fn undefined_transition_errors() {
        let m = branch_machine();
        assert!(m
            .transition(&State::new("merged"), "submit", &BTreeMap::new())
            .is_err());
    }

    #[test]
    fn guard_can_block() {
        fn deny(_: &BTreeMap<String, lattice_core::Value>) -> Result<(), Error> {
        ...
        assert!(m
            .transition(&State::new("a"), "go", &BTreeMap::new())
            .is_err());
    }
