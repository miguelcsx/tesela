...
use lattice_core::{DataType, Error, Value};
use lattice_ir::{ObjectType, Property};
...
impl UploadMappingEngine {
    ...
    pub fn map_column(&self, col: &str, ot: &ObjectType) -> Option<String> {
        ...
        if self.heuristic {
            if let Some(best) = fuzzy_match(col, &ot.properties) {
                return Some(best);
            }
        }
        None
...
pub fn validate_records(records: &[lattice_ir::Record], ot: &ObjectType) -> Result<(), Error> {
    ...
    for (idx, record) in records.iter().enumerate() {
        for prop in &ot.properties {
            ...
            if let Some(value) = record.values.get(key) {
                if value.is_null() && prop.nullable != Some(true) {
                    errors.push(format!(
