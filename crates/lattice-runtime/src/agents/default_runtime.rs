...
use lattice_core::{ApiName, Error, Operation};
...
pub struct DefaultAgentRuntime {
    ...
    runs: Mutex<BTreeMap<String, lattice_ir::AgentRun>>,
}
...
impl DefaultAgentRuntime {
    ...
    fn build_system_prompt(&self, agent: &lattice_ir::Agent, tools: &[ToolDef]) -> String {
    ...
    fn assemble_tools(&self, _agent: &lattice_ir::Agent, actor: &Actor) -> Vec<ToolDef> {
...
impl AgentRuntime for DefaultAgentRuntime {
    fn start_run(
        ...
        if agent.requires_approval == Some(true) {
            if let Some(approval) = &self.approval_provider {
