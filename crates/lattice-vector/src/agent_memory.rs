...
use lattice_runtime::ports::{AgentMemoryStore, VectorBackend, VectorSearchQuery};
use std::sync::Arc;
...
impl VectorAgentMemoryStore {
    ...
    fn memory_key(namespace: &str, key: &str) -> lattice_core::Value {
        lattice_core::Value(serde_json::Value::String(format!("{}:{}", namespace, key)))
    }

    fn memory_object_type() -> lattice_core::ApiName {
        lattice_core::ApiName::new_unchecked(MEMORY_OBJECT_TYPE)
    }
}
