...
impl McpServer {
    ...
    pub fn handle(&self, req: JsonRpcRequest) -> JsonRpcResponse {
        ...
        match self.dispatch(&req) {
            ...
            Err(e) => {
                let code = tesela_error_to_rpc_code(&e);
    ...
    fn dispatch(&self, req: &JsonRpcRequest) -> Result<serde_json::Value, Error> {
        match req.method.as_str() {
            "initialize" => Ok(serde_json::json!({
                ...
                "capabilities": { "tools": {} },
                "serverInfo": { "name": "tesela-mcp", "version": "0.1.0" }
            })),
    ...
    pub async fn serve(self, addr: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        tracing::info!("Tesela MCP server listening on {}", addr);
...
fn tesela_error_to_rpc_code(e: &Error) -> i32 {
    match e {
