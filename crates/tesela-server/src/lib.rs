...
//! HTTP REST server for Tesela runtime.
//!
//! Provides an Axum-based HTTP server exposing all Tesela runtime operations
//! through a versioned REST API. Authentication is handled via an optional
//! [`tesela_runtime::ports::ActorResolver`] middleware.
...
impl Server {
    ...
    pub async fn serve(self, addr: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        info!("Tesela HTTP server listening on {}", addr);
