...
impl Runtime {
    ...
    pub fn apply_spec(&self, new_spec: Spec) -> Result<tesela_compiler::Diff, Error> {
        let old_snap = self.ontology()?;
        let diff = tesela_compiler::compute_diff(&old_snap.spec, &new_spec);

        if let Some(meta) = &self.meta_store {
            let hash = tesela_compiler::hash_spec(&new_spec);
