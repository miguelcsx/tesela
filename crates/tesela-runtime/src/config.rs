...
///
/// Example: key `"server.port"` with prefix `"TESELA"` → `TESELA_SERVER_PORT`.
...
impl EnvConfigSource {
    ...
    /// Create a source that prepends `prefix` (e.g. `"TESELA"`).
...
mod tests {
    ...
    fn env_config_source_maps_key() {
        unsafe { std::env::set_var("TESELA_SERVER_PORT", "9090") };
        let s = EnvConfigSource::with_prefix("TESELA");
        assert_eq!(s.get("server.port").unwrap(), Some("9090".to_string()));
        unsafe { std::env::remove_var("TESELA_SERVER_PORT") };
    }
}
