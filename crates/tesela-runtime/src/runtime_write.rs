...
use tesela_ir::{ExecutionMode, MutationResult, Page, PipelineResult};
...
impl Runtime {
    ...
    pub fn mutate(
        ...
        self.metric_counter("tesela_mutate_total", "Total mutate calls", &[])
            .inc(1);
        self.metric_histogram("tesela_mutate_duration_ms", "Mutate latency", &[])
            .record(elapsed_ms);
            ...
            sort: os
                ...
                .map(|s| Sort {
                    ...
                    direction: match s.direction {
                        tesela_ir::SortDirection::Asc => "asc".to_string(),
                        tesela_ir::SortDirection::Desc => "desc".to_string(),
                    },
                })
                .collect(),
            ..Query::default()
    ...
    pub fn execute_action(
        ...
        action_name: &ApiName,
        input: Value,
    ) -> Result<tesela_ir::ActionResult, Error> {
    ...
    pub fn get_agent_run(
        ...
        _actor: &Actor,
        run_id: &str,
    ) -> Result<tesela_ir::AgentRun, Error> {
        let runtime = self
            .agent_runtime
            .as_ref()
            .ok_or_else(|| Error::internal("no agent runtime configured"))?;
        ...
        if run.status == "completed"
            && let Some(evaluator) = &self.agent_evaluator
            && let Ok(result) = evaluator.evaluate(&run, &ctx)
        {
