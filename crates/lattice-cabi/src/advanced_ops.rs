...
fn runtime_from_handle(handle: u64) -> Result<std::sync::Arc<lattice_runtime::Runtime>, String> {
    let ht = handles()
        .lock()
...
pub unsafe extern "C" fn lattice_runtime_vector_search_json(
    ...
    let obj_str = body
        ...
        .and_then(|v| v.as_str())
        .unwrap_or("");
    let object_type = match obj_str.parse::<lattice_core::ApiName>() {
        Ok(n) => n,
        Err(e) => {
    ...
    let query_vector: Vec<f32> = body
        ...
        .unwrap_or_default();
    let query = lattice_runtime::ports::VectorSearchQuery {
        object_type,
        query_vector,
...
pub unsafe extern "C" fn lattice_runtime_compose_object_sets_json(
    ...
    let names: Vec<lattice_core::ApiName> = body
        .get("names")
        .and_then(|v| v.as_array())
        .map(|arr| {
            arr.iter()
...
pub unsafe extern "C" fn lattice_runtime_cross_search_json(
    ...
    for item in &body {
        ...
        let ot_str = item
            ...
            .and_then(|v| v.as_str())
            .unwrap_or("");
        let ds = match ds_str.parse::<lattice_core::ApiName>() {
            Ok(n) => n,
            Err(e) => {
        ...
        let ot = match ot_str.parse::<lattice_core::ApiName>() {
            Ok(n) => n,
            Err(e) => {
                ht.set_error(&format!("invalid object_type: {}", e));
                return LatticeBuffer::empty();
        ...
        let query = lattice_runtime::query::Query {
            filter: item
                .get("filter")
                .and_then(|v| serde_json::from_value(v.clone()).ok()),
            sort: item
        ...
        queries.push(lattice_runtime::ports::FederatedQuery {
            datasource: ds,
            object_type: ot,
            query,
        });
